...
[TASK]
====
* Create files `encoding/huffman/tree-encoding.cpp` and `encoding/huffman/tree-encoding.h`.
...
* While recursion is never mandatory, it will considerably simplify the implementation.
====
...
*** Write access is needed.
*** No ownership transfer.
* It returns a `Node<Datum>`.
** The tree will need to be created on the heap.
** While building the tree, the `decode_tree` function has ownership, but at the end needs to give it away to the caller of the function.
